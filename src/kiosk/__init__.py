...
Das Basis-Verzeichnis `kiosk` (direkt unter `src`) wird durch `__init__.py` als Modul
für das Projekt deklariert, damit `kiosk` als oberster Namensraum bei den Imports
verwendet werden kann.

Außerdem kann mit dem Package Manager _uv_ das Modul als Skript aufgerufen werden:
`uv run kiosk`. Siehe `project.scripts` in `pyproject.toml`.

Desweiteren wird für _uvicorn_ die 'Package-Level' Variable `app` initialisiert.
Dann lautet der Aufruf:

```powershell
uv run uvicorn src.kiosk:app --ssl-certfile=src\kiosk\config\resources\tls\certificate.crt --ssl-keyfile=src\kiosk\config\resources\tls\key.pem
```
...
uvicorn src.kiosk:app --ssl-certfile=src\kiosk\config\resources\tls\certificate.crt --ssl-keyfile=src\kiosk\config\resources\tls\key.pem
```
...
```powershell
uv run fastapi dev src\kiosk
```
"""  # noqa: E501

from kiosk.asgi_server import run
from kiosk.fastapi_app import app
