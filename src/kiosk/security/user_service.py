...
from kiosk.config import keycloak_admin_config
...
class UserService:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            roles_kiosk = [role for role in roles if role["name"] == "kiosk"]
            # Rolle 'kiosk' in Keycloak fuer KeycloakAdmin.assign_client_role() und
            # ist ein dict mit Schluesseln "id", "name", "description", ...
            self.rolle_kiosk: Any = roles_kiosk[0]
            logger.debug("rolle_kiosk={}", self.rolle_kiosk)
        except KeycloakConnectionError:
            logger.error("Keine Verbindung zu Keycloak! Ist Keycloak gestartet?")
            self.client_uuid = "N/A"
            self.rolle_kiosk = None
