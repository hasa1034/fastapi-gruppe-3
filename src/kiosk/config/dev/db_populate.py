...
from kiosk.config.config import resources_path
...
from kiosk.config.dev_modus import dev_db_populate
from kiosk.repository import engine
...
class DbPopulateService:
    ...
    def _load_csv_files(self) -> None:
        logger.debug("begin")
        tabellen: Final = ["kiosk", "betreiber", "produkt"]
        csv_path: Final = "/init/kiosk/csv"
        # siehe extras/compose/postgres/compose.init.yml
        with self.engine_admin.connect() as connection:
            connection.execute(text("SET search_path TO kiosk;"))
