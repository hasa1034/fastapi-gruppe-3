...
from kiosk.banner import banner
from kiosk.config import (
    dev_db_populate,
    dev_keycloak_populate,
)
from kiosk.config.dev.db_populate import db_populate
from kiosk.config.dev.db_populate_router import router as db_populate_router
from kiosk.config.dev.keycloak_populate import keycloak_populate
from kiosk.config.dev.keycloak_populate_router import (
    router as keycloak_populate_router,
)
from kiosk.graphql_api import graphql_router
from kiosk.problem_details import create_problem_details
from kiosk.repository.session_factory import engine
from kiosk.router import (
    health_router,
    kiosk_router,
    kiosk_write_router,
    shutdown_router,
)
from kiosk.security import AuthorizationError, LoginError, set_response_headers
from kiosk.security import router as auth_router
...
# R E S T
# --------------------------------------------------------------------------------------
app.include_router(kiosk_router, prefix="/rest")
app.include_router(kiosk_write_router, prefix="/rest")
...
def favicon() -> FileResponse:
    ...
    favicon_path: Final = Path("kiosk") / "static" / file_name
...
def email_exists_error_handler(_request: Request, err: EmailExistsError) -> Response:
    ...
    :param err: Exception, falls die Emailadresse des neuen oder zu ändernden Kiosks
        bereits existiert
